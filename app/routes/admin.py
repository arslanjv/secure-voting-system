...
SECURITY FIXES:
- VULN-001: Updated tallying to use hybrid RSA+AES decryption
"""
from flask import Blueprint, render_template, redirect, url_for, flash, request, current_app, send_file
from flask_login import login_required, current_user
from app.models import db, Election, Candidate, Vote, User, AuditLog, ElectionStatus, UserRole, ElectionKeyPair
...
def delete_election(election_id):
    ...
    try:
        ...
        ElectionKeyPair.query.filter_by(election_id=election_id).delete()
...
def activate_election(election_id):
    """Activate election and generate encryption keys"""
    election = Election.query.get_or_404(election_id)
    ...
    try:
        # Generate RSA-4096 keypair for this election (VULN-001 fix)
        master_key = current_app.config.get('MASTER_ENCRYPTION_KEY')
        if not master_key:
            flash('Server encryption keys not configured. Please contact administrator.', 'danger')
            return redirect(url_for('admin.view_election', election_id=election_id))
        
        # Check if keypair already exists
        existing_keypair = ElectionKeyPair.query.filter_by(election_id=election_id).first()
        ...
        # Generate Ed25519 signature keypair (legacy support)
        ...
        flash('Election activated successfully! RSA-4096 encryption keys generated.', 'success')
        
    except Exception as e:
        db.session.rollback()
        current_app.logger.error(f"Election activation error: {str(e)}")
        flash('Error activating election. Please try again.', 'danger')
...
def decrypt_vote_hybrid(vote, keypair):
    """
    Decrypt a vote using hybrid RSA+AES encryption.
    
    Args:
        vote: Vote object with encrypted_vote, encrypted_key, vote_nonce (iv)
        keypair: ElectionKeyPair with private key
        
    Returns:
        str: Decrypted vote data as JSON string
    """
    # Get decrypted private key
    private_key_pem = keypair.get_private_key()
    
    # Decrypt using hybrid encryption
    decrypted = CryptoUtils.decrypt_hybrid_vote(
        encrypted_vote_b64=vote.encrypted_vote,
        encrypted_key_b64=vote.encrypted_key,
        iv_b64=vote.vote_nonce,
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            for vote in votes:
                try:
                    ...
                    if vote.is_hybrid_encrypted() and keypair:
                        # New hybrid RSA+AES encryption (VULN-001 fix)
                        decrypted_data = decrypt_vote_hybrid(vote, keypair)
                        hybrid_votes += 1
                    else:
                    ...
                    current_app.logger.error(f"Vote decryption error for vote {vote.id}: {str(e)}")
                    decryption_errors += 1
            ...
            log_audit('ELECTION_TALLIED',
                     f'Election tallied: {election.title} ({hybrid_votes} hybrid, {legacy_votes} legacy, {decryption_errors} errors)',
                     'INFO', 'election', election.id)

            flash(f'Election tallied successfully! ({hybrid_votes} hybrid + {legacy_votes} legacy votes)', 'success')
...
def invite_single():
    ...
    if form.validate_on_submit():
        ...
        if success:
            ...
            flash(f'Invite created but email failed to send', 'warning')
